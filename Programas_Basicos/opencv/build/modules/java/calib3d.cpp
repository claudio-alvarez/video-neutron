...
{
    try {
        LOGD("calib3d::StereoSGBM_10()");
        
        StereoSGBM* _retval_ = new StereoSGBM(  );
        return (jlong) _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::StereoSGBM_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::StereoSGBM_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::StereoSGBM_10()}");
        return 0;
    }
}
  ...
  (JNIEnv* env, jclass , jint minDisparity, jint numDisparities, jint SADWindowSize, jint P1, jint P2, jint disp12MaxDiff, jint preFilterCap, jint uniquenessRatio, jint speckleWindowSize, jint speckleRange, jboolean fullDP)
{
    try {
        ...
        LOGD("calib3d::StereoSGBM_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::StereoSGBM_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::StereoSGBM_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::StereoSGBM_12()");
        
        StereoSGBM* _retval_ = new StereoSGBM( (int)minDisparity, (int)numDisparities, (int)SADWindowSize );
        return (jlong) _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::StereoSGBM_12() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::StereoSGBM_12() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::StereoSGBM_12()}");
        return 0;
    }
}
  ...
  (JNIEnv* env, jclass , jlong self, jlong left_nativeObj, jlong right_nativeObj, jlong disp_nativeObj)
{
    try {
        ...
        LOGD("calib3d::compute_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::compute_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::compute_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1minDisparity_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        int _retval_ = me->minDisparity;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1minDisparity_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1minDisparity_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1minDisparity_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1minDisparity_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->minDisparity = ( (int)minDisparity );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1minDisparity_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1minDisparity_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1minDisparity_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1numberOfDisparities_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        int _retval_ = me->numberOfDisparities;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1numberOfDisparities_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1numberOfDisparities_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1numberOfDisparities_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1numberOfDisparities_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->numberOfDisparities = ( (int)numberOfDisparities );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1numberOfDisparities_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1numberOfDisparities_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1numberOfDisparities_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1SADWindowSize_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        int _retval_ = me->SADWindowSize;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1SADWindowSize_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1SADWindowSize_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1SADWindowSize_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1SADWindowSize_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->SADWindowSize = ( (int)SADWindowSize );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1SADWindowSize_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1SADWindowSize_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1SADWindowSize_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1preFilterCap_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        int _retval_ = me->preFilterCap;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1preFilterCap_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1preFilterCap_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1preFilterCap_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1preFilterCap_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->preFilterCap = ( (int)preFilterCap );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1preFilterCap_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1preFilterCap_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1preFilterCap_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1uniquenessRatio_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        int _retval_ = me->uniquenessRatio;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1uniquenessRatio_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1uniquenessRatio_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1uniquenessRatio_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1uniquenessRatio_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->uniquenessRatio = ( (int)uniquenessRatio );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1uniquenessRatio_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1uniquenessRatio_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1uniquenessRatio_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1P1_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        int _retval_ = me->P1;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1P1_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1P1_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1P1_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1P1_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->P1 = ( (int)P1 );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1P1_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1P1_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1P1_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1P2_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        int _retval_ = me->P2;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1P2_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1P2_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1P2_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1P2_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->P2 = ( (int)P2 );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1P2_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1P2_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1P2_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1speckleWindowSize_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        int _retval_ = me->speckleWindowSize;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1speckleWindowSize_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1speckleWindowSize_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1speckleWindowSize_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1speckleWindowSize_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->speckleWindowSize = ( (int)speckleWindowSize );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1speckleWindowSize_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1speckleWindowSize_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1speckleWindowSize_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1speckleRange_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        int _retval_ = me->speckleRange;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1speckleRange_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1speckleRange_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1speckleRange_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1speckleRange_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->speckleRange = ( (int)speckleRange );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1speckleRange_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1speckleRange_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1speckleRange_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1disp12MaxDiff_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        int _retval_ = me->disp12MaxDiff;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1disp12MaxDiff_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1disp12MaxDiff_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1disp12MaxDiff_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1disp12MaxDiff_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->disp12MaxDiff = ( (int)disp12MaxDiff );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1disp12MaxDiff_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1disp12MaxDiff_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1disp12MaxDiff_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::get_1fullDP_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        bool _retval_ = me->fullDP;//(  );
        return _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::get_1fullDP_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::get_1fullDP_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::get_1fullDP_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::set_1fullDP_10()");
        StereoSGBM* me = (StereoSGBM*) self; //TODO: check for NULL
        me->fullDP = ( (bool)fullDP );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::set_1fullDP_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::set_1fullDP_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::set_1fullDP_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::StereoBM_10()");
        
        StereoBM* _retval_ = new StereoBM(  );
        return (jlong) _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::StereoBM_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::StereoBM_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::StereoBM_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::StereoBM_11()");
        
        StereoBM* _retval_ = new StereoBM( (int)preset, (int)ndisparities, (int)SADWindowSize );
        return (jlong) _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::StereoBM_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::StereoBM_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::StereoBM_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::StereoBM_12()");
        
        StereoBM* _retval_ = new StereoBM( (int)preset );
        return (jlong) _retval_;
    } catch(cv::Exception e) {
        LOGD("calib3d::StereoBM_12() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::StereoBM_12() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::StereoBM_12()}");
        return 0;
    }
}
  ...
  (JNIEnv* env, jclass , jlong self, jlong left_nativeObj, jlong right_nativeObj, jlong disparity_nativeObj)
{
    try {
        ...
        LOGD("calib3d::compute_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::compute_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::compute_11()}");
        return;
    }
}
  ...
  (JNIEnv* env, jclass , jlong src_nativeObj, jlong mtxR_nativeObj, jlong mtxQ_nativeObj, jlong Qx_nativeObj, jlong Qy_nativeObj, jlong Qz_nativeObj)
{
    try {
        ...
        LOGD("calib3d::RQDecomp3x3_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::RQDecomp3x3_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::RQDecomp3x3_10()}");
        return 0;
    }
}
  ...
  (JNIEnv* env, jclass , jlong src_nativeObj, jlong mtxR_nativeObj, jlong mtxQ_nativeObj)
{
    try {
        ...
        LOGD("calib3d::RQDecomp3x3_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::RQDecomp3x3_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::RQDecomp3x3_11()}");
        return 0;
    }
}
  ...
  (JNIEnv* env, jclass , jlong src_nativeObj, jlong dst_nativeObj, jlong jacobian_nativeObj)
{
    try {
        ...
        LOGD("calib3d::Rodrigues_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::Rodrigues_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::Rodrigues_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::Rodrigues_11()");
        Mat& src = *((Mat*)src_nativeObj);
        Mat& dst = *((Mat*)dst_nativeObj);
        cv::Rodrigues( src, dst );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::Rodrigues_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::Rodrigues_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::Rodrigues_11()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::calibrateCamera_10()");
        ...
        LOGD("calib3d::calibrateCamera_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::calibrateCamera_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::calibrateCamera_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::calibrateCamera_11()");
        vector<Mat> objectPoints;
        Mat& objectPoints_mat = *((Mat*)objectPoints_mat_nativeObj);
        Mat_to_vector_Mat( objectPoints_mat, objectPoints );
        vector<Mat> imagePoints;
        ...
        LOGD("calib3d::calibrateCamera_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::calibrateCamera_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::calibrateCamera_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::calibrateCamera_12()");
        vector<Mat> objectPoints;
        Mat& objectPoints_mat = *((Mat*)objectPoints_mat_nativeObj);
        Mat_to_vector_Mat( objectPoints_mat, objectPoints );
        vector<Mat> imagePoints;
        ...
        LOGD("calib3d::calibrateCamera_12() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::calibrateCamera_12() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::calibrateCamera_12()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::calibrationMatrixValues_10()");
        Mat& cameraMatrix = *((Mat*)cameraMatrix_nativeObj);
        Size imageSize((int)imageSize_width, (int)imageSize_height);
        double fovx;
        double fovy;
        ...
        LOGD("calib3d::calibrationMatrixValues_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::calibrationMatrixValues_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::calibrationMatrixValues_10()}");
        return;
    }
}
  ...
  (JNIEnv* env, jclass , jlong rvec1_nativeObj, jlong tvec1_nativeObj, jlong rvec2_nativeObj, jlong tvec2_nativeObj, jlong rvec3_nativeObj, jlong tvec3_nativeObj, jlong dr3dr1_nativeObj, jlong dr3dt1_nativeObj, jlong dr3dr2_nativeObj, jlong dr3dt2_nativeObj, jlong dt3dr1_nativeObj, jlong dt3dt1_nativeObj, jlong dt3dr2_nativeObj, jlong dt3dt2_nativeObj)
{
    try {
        ...
        LOGD("calib3d::composeRT_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::composeRT_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::composeRT_10()}");
        return;
    }
}
  ...
  (JNIEnv* env, jclass , jlong rvec1_nativeObj, jlong tvec1_nativeObj, jlong rvec2_nativeObj, jlong tvec2_nativeObj, jlong rvec3_nativeObj, jlong tvec3_nativeObj)
{
    try {
        ...
        LOGD("calib3d::composeRT_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::composeRT_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::composeRT_11()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::computeCorrespondEpilines_10()");
        ...
        LOGD("calib3d::computeCorrespondEpilines_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::computeCorrespondEpilines_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::computeCorrespondEpilines_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::convertPointsFromHomogeneous_10()");
        Mat& src = *((Mat*)src_nativeObj);
        Mat& dst = *((Mat*)dst_nativeObj);
        cv::convertPointsFromHomogeneous( src, dst );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::convertPointsFromHomogeneous_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::convertPointsFromHomogeneous_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::convertPointsFromHomogeneous_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::convertPointsToHomogeneous_10()");
        Mat& src = *((Mat*)src_nativeObj);
        Mat& dst = *((Mat*)dst_nativeObj);
        cv::convertPointsToHomogeneous( src, dst );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::convertPointsToHomogeneous_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::convertPointsToHomogeneous_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::convertPointsToHomogeneous_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::correctMatches_10()");
        Mat& F = *((Mat*)F_nativeObj);
        ...
        LOGD("calib3d::correctMatches_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::correctMatches_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::correctMatches_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::decomposeProjectionMatrix_10()");
        ...
        LOGD("calib3d::decomposeProjectionMatrix_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::decomposeProjectionMatrix_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::decomposeProjectionMatrix_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::decomposeProjectionMatrix_11()");
        Mat& projMatrix = *((Mat*)projMatrix_nativeObj);
        Mat& cameraMatrix = *((Mat*)cameraMatrix_nativeObj);
        Mat& rotMatrix = *((Mat*)rotMatrix_nativeObj);
        Mat& transVect = *((Mat*)transVect_nativeObj);
        ...
        LOGD("calib3d::decomposeProjectionMatrix_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::decomposeProjectionMatrix_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::decomposeProjectionMatrix_11()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::drawChessboardCorners_10()");
        ...
        LOGD("calib3d::drawChessboardCorners_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::drawChessboardCorners_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::drawChessboardCorners_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::estimateAffine3D_10()");
        Mat& src = *((Mat*)src_nativeObj);
        Mat& dst = *((Mat*)dst_nativeObj);
        ...
        LOGD("calib3d::estimateAffine3D_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::estimateAffine3D_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::estimateAffine3D_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::estimateAffine3D_11()");
        Mat& src = *((Mat*)src_nativeObj);
        Mat& dst = *((Mat*)dst_nativeObj);
        Mat& out = *((Mat*)out_nativeObj);
        Mat& inliers = *((Mat*)inliers_nativeObj);
        ...
        LOGD("calib3d::estimateAffine3D_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::estimateAffine3D_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::estimateAffine3D_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::filterSpeckles_10()");
        Mat& img = *((Mat*)img_nativeObj);
        Mat& buf = *((Mat*)buf_nativeObj);
        cv::filterSpeckles( img, (double)newVal, (int)maxSpeckleSize, (double)maxDiff, buf );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::filterSpeckles_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::filterSpeckles_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::filterSpeckles_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::filterSpeckles_11()");
        Mat& img = *((Mat*)img_nativeObj);
        cv::filterSpeckles( img, (double)newVal, (int)maxSpeckleSize, (double)maxDiff );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::filterSpeckles_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::filterSpeckles_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::filterSpeckles_11()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::findChessboardCorners_10()");
        vector<Point2f> corners;
        Mat& corners_mat = *((Mat*)corners_mat_nativeObj);
        Mat& image = *((Mat*)image_nativeObj);
        Size patternSize((int)patternSize_width, (int)patternSize_height);
        ...
        LOGD("calib3d::findChessboardCorners_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::findChessboardCorners_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::findChessboardCorners_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::findChessboardCorners_11()");
        vector<Point2f> corners;
        Mat& corners_mat = *((Mat*)corners_mat_nativeObj);
        Mat& image = *((Mat*)image_nativeObj);
        Size patternSize((int)patternSize_width, (int)patternSize_height);
        ...
        LOGD("calib3d::findChessboardCorners_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::findChessboardCorners_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::findChessboardCorners_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::findCirclesGridDefault_10()");
        Mat& image = *((Mat*)image_nativeObj);
        Size patternSize((int)patternSize_width, (int)patternSize_height);
        ...
        LOGD("calib3d::findCirclesGridDefault_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::findCirclesGridDefault_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::findCirclesGridDefault_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::findCirclesGridDefault_11()");
        Mat& image = *((Mat*)image_nativeObj);
        Size patternSize((int)patternSize_width, (int)patternSize_height);
        Mat& centers = *((Mat*)centers_nativeObj);
        ...
        LOGD("calib3d::findCirclesGridDefault_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::findCirclesGridDefault_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::findCirclesGridDefault_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::findFundamentalMat_10()");
        ...
        LOGD("calib3d::findFundamentalMat_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::findFundamentalMat_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::findFundamentalMat_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::findFundamentalMat_11()");
        vector<Point2f> points1;
        Mat& points1_mat = *((Mat*)points1_mat_nativeObj);
        Mat_to_vector_Point2f( points1_mat, points1 );
        vector<Point2f> points2;
        ...
        LOGD("calib3d::findFundamentalMat_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::findFundamentalMat_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::findFundamentalMat_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::findFundamentalMat_12()");
        vector<Point2f> points1;
        Mat& points1_mat = *((Mat*)points1_mat_nativeObj);
        Mat_to_vector_Point2f( points1_mat, points1 );
        vector<Point2f> points2;
        ...
        LOGD("calib3d::findFundamentalMat_12() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::findFundamentalMat_12() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::findFundamentalMat_12()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::findHomography_10()");
        ...
        LOGD("calib3d::findHomography_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::findHomography_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::findHomography_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::findHomography_11()");
        vector<Point2f> srcPoints;
        Mat& srcPoints_mat = *((Mat*)srcPoints_mat_nativeObj);
        Mat_to_vector_Point2f( srcPoints_mat, srcPoints );
        vector<Point2f> dstPoints;
        ...
        LOGD("calib3d::findHomography_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::findHomography_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::findHomography_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::findHomography_12()");
        vector<Point2f> srcPoints;
        Mat& srcPoints_mat = *((Mat*)srcPoints_mat_nativeObj);
        Mat_to_vector_Point2f( srcPoints_mat, srcPoints );
        vector<Point2f> dstPoints;
        ...
        LOGD("calib3d::findHomography_12() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::findHomography_12() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::findHomography_12()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::getOptimalNewCameraMatrix_10()");
        Mat& cameraMatrix = *((Mat*)cameraMatrix_nativeObj);
        Mat& distCoeffs = *((Mat*)distCoeffs_nativeObj);
        Size imageSize((int)imageSize_width, (int)imageSize_height);
        ...
        LOGD("calib3d::getOptimalNewCameraMatrix_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::getOptimalNewCameraMatrix_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::getOptimalNewCameraMatrix_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::getOptimalNewCameraMatrix_11()");
        Mat& cameraMatrix = *((Mat*)cameraMatrix_nativeObj);
        Mat& distCoeffs = *((Mat*)distCoeffs_nativeObj);
        Size imageSize((int)imageSize_width, (int)imageSize_height);
        ...
        LOGD("calib3d::getOptimalNewCameraMatrix_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::getOptimalNewCameraMatrix_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::getOptimalNewCameraMatrix_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::getValidDisparityROI_10()");
        ...
        LOGD("calib3d::getValidDisparityROI_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::getValidDisparityROI_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::getValidDisparityROI_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::initCameraMatrix2D_10()");
        ...
        LOGD("calib3d::initCameraMatrix2D_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::initCameraMatrix2D_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::initCameraMatrix2D_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::initCameraMatrix2D_11()");
        vector< vector<Point3f> > objectPoints;
        Mat& objectPoints_mat = *((Mat*)objectPoints_mat_nativeObj);
        Mat_to_vector_vector_Point3f( objectPoints_mat, objectPoints );
        vector< vector<Point2f> > imagePoints;
        ...
        LOGD("calib3d::initCameraMatrix2D_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::initCameraMatrix2D_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::initCameraMatrix2D_11()}");
        return 0;
    }
}
  ...
  (JNIEnv* env, jclass , jlong A_nativeObj, jlong B_nativeObj, jlong dABdA_nativeObj, jlong dABdB_nativeObj)
{
    try {
        ...
        LOGD("calib3d::matMulDeriv_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::matMulDeriv_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::matMulDeriv_10()}");
        return;
    }
}
  ...
  (JNIEnv* env, jclass , jlong objectPoints_mat_nativeObj, jlong rvec_nativeObj, jlong tvec_nativeObj, jlong cameraMatrix_nativeObj, jlong distCoeffs_mat_nativeObj, jlong imagePoints_mat_nativeObj, jlong jacobian_nativeObj, jdouble aspectRatio)
{
    try {
        ...
        LOGD("calib3d::projectPoints_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::projectPoints_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::projectPoints_10()}");
        return;
    }
}
  ...
  (JNIEnv* env, jclass , jlong objectPoints_mat_nativeObj, jlong rvec_nativeObj, jlong tvec_nativeObj, jlong cameraMatrix_nativeObj, jlong distCoeffs_mat_nativeObj, jlong imagePoints_mat_nativeObj)
{
    try {
        ...
        LOGD("calib3d::projectPoints_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::projectPoints_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::projectPoints_11()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::rectify3Collinear_10()");
        vector<Mat> imgpt1;
        ...
        LOGD("calib3d::rectify3Collinear_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::rectify3Collinear_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::rectify3Collinear_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::reprojectImageTo3D_10()");
        Mat& disparity = *((Mat*)disparity_nativeObj);
        ...
        LOGD("calib3d::reprojectImageTo3D_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::reprojectImageTo3D_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::reprojectImageTo3D_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::reprojectImageTo3D_11()");
        Mat& disparity = *((Mat*)disparity_nativeObj);
        Mat& _3dImage = *((Mat*)_3dImage_nativeObj);
        Mat& Q = *((Mat*)Q_nativeObj);
        ...
        LOGD("calib3d::reprojectImageTo3D_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::reprojectImageTo3D_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::reprojectImageTo3D_11()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::reprojectImageTo3D_12()");
        Mat& disparity = *((Mat*)disparity_nativeObj);
        Mat& _3dImage = *((Mat*)_3dImage_nativeObj);
        Mat& Q = *((Mat*)Q_nativeObj);
        ...
        LOGD("calib3d::reprojectImageTo3D_12() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::reprojectImageTo3D_12() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::reprojectImageTo3D_12()}");
        return;
    }
}
  ...
  (JNIEnv* env, jclass , jlong objectPoints_mat_nativeObj, jlong imagePoints_mat_nativeObj, jlong cameraMatrix_nativeObj, jlong distCoeffs_mat_nativeObj, jlong rvec_nativeObj, jlong tvec_nativeObj, jboolean useExtrinsicGuess, jint flags)
{
    try {
        ...
        LOGD("calib3d::solvePnP_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::solvePnP_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::solvePnP_10()}");
        return 0;
    }
}
  ...
  (JNIEnv* env, jclass , jlong objectPoints_mat_nativeObj, jlong imagePoints_mat_nativeObj, jlong cameraMatrix_nativeObj, jlong distCoeffs_mat_nativeObj, jlong rvec_nativeObj, jlong tvec_nativeObj)
{
    try {
        ...
        LOGD("calib3d::solvePnP_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::solvePnP_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::solvePnP_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::solvePnPRansac_10()");
        vector<Point3f> objectPoints;
        Mat& objectPoints_mat = *((Mat*)objectPoints_mat_nativeObj);
        Mat_to_vector_Point3f( objectPoints_mat, objectPoints );
        vector<Point2f> imagePoints;
        ...
        LOGD("calib3d::solvePnPRansac_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::solvePnPRansac_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::solvePnPRansac_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::solvePnPRansac_11()");
        vector<Point3f> objectPoints;
        Mat& objectPoints_mat = *((Mat*)objectPoints_mat_nativeObj);
        Mat_to_vector_Point3f( objectPoints_mat, objectPoints );
        vector<Point2f> imagePoints;
        ...
        LOGD("calib3d::solvePnPRansac_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::solvePnPRansac_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::solvePnPRansac_11()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::stereoCalibrate_10()");
        vector<Mat> objectPoints;
        Mat& objectPoints_mat = *((Mat*)objectPoints_mat_nativeObj);
        Mat_to_vector_Mat( objectPoints_mat, objectPoints );
        ...
        LOGD("calib3d::stereoCalibrate_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::stereoCalibrate_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::stereoCalibrate_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::stereoCalibrate_11()");
        vector<Mat> objectPoints;
        Mat& objectPoints_mat = *((Mat*)objectPoints_mat_nativeObj);
        Mat_to_vector_Mat( objectPoints_mat, objectPoints );
        vector<Mat> imagePoints1;
        ...
        LOGD("calib3d::stereoCalibrate_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::stereoCalibrate_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::stereoCalibrate_11()}");
        return 0;
    }
}
  ...
  (JNIEnv* env, jclass , jlong cameraMatrix1_nativeObj, jlong distCoeffs1_nativeObj, jlong cameraMatrix2_nativeObj, jlong distCoeffs2_nativeObj, jdouble imageSize_width, jdouble imageSize_height, jlong R_nativeObj, jlong T_nativeObj, jlong R1_nativeObj, jlong R2_nativeObj, jlong P1_nativeObj, jlong P2_nativeObj, jlong Q_nativeObj, jint flags, jdouble alpha, jdouble newImageSize_width, jdouble newImageSize_height, jdoubleArray validPixROI1_out, jdoubleArray validPixROI2_out)
{
    try {
        ...
        LOGD("calib3d::stereoRectify_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::stereoRectify_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::stereoRectify_10()}");
        return;
    }
}
  ...
  (JNIEnv* env, jclass , jlong cameraMatrix1_nativeObj, jlong distCoeffs1_nativeObj, jlong cameraMatrix2_nativeObj, jlong distCoeffs2_nativeObj, jdouble imageSize_width, jdouble imageSize_height, jlong R_nativeObj, jlong T_nativeObj, jlong R1_nativeObj, jlong R2_nativeObj, jlong P1_nativeObj, jlong P2_nativeObj, jlong Q_nativeObj)
{
    try {
        ...
        LOGD("calib3d::stereoRectify_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::stereoRectify_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::stereoRectify_11()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::stereoRectifyUncalibrated_10()");
        Mat& points1 = *((Mat*)points1_nativeObj);
        Mat& points2 = *((Mat*)points2_nativeObj);
        Mat& F = *((Mat*)F_nativeObj);
        ...
        LOGD("calib3d::stereoRectifyUncalibrated_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::stereoRectifyUncalibrated_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::stereoRectifyUncalibrated_10()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::stereoRectifyUncalibrated_11()");
        Mat& points1 = *((Mat*)points1_nativeObj);
        Mat& points2 = *((Mat*)points2_nativeObj);
        Mat& F = *((Mat*)F_nativeObj);
        Size imgSize((int)imgSize_width, (int)imgSize_height);
        ...
        LOGD("calib3d::stereoRectifyUncalibrated_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return 0;
    } catch (...) {
        LOGD("calib3d::stereoRectifyUncalibrated_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::stereoRectifyUncalibrated_11()}");
        return 0;
    }
}
...
{
    try {
        LOGD("calib3d::triangulatePoints_10()");
        ...
        LOGD("calib3d::triangulatePoints_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::triangulatePoints_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::triangulatePoints_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::validateDisparity_10()");
        Mat& disparity = *((Mat*)disparity_nativeObj);
        ...
        LOGD("calib3d::validateDisparity_10() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::validateDisparity_10() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::validateDisparity_10()}");
        return;
    }
}
...
{
    try {
        LOGD("calib3d::validateDisparity_11()");
        Mat& disparity = *((Mat*)disparity_nativeObj);
        Mat& cost = *((Mat*)cost_nativeObj);
        cv::validateDisparity( disparity, cost, (int)minDisparity, (int)numberOfDisparities );
        return;
    } catch(cv::Exception e) {
        LOGD("calib3d::validateDisparity_11() catched cv::Exception: %s", e.what());
        jclass je = env->FindClass("org/opencv/core/CvException");
        if(!je) je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, e.what());
        return;
    } catch (...) {
        LOGD("calib3d::validateDisparity_11() catched unknown exception (...)");
        jclass je = env->FindClass("java/lang/Exception");
        env->ThrowNew(je, "Unknown exception in JNI code {calib3d::validateDisparity_11()}");
        return;
    }
}
